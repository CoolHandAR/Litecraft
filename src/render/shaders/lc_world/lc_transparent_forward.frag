...
void main()
{
    ...
    vec4 Albedo = texture(texture_atlas, texCoords);
    ...
    vec3 Normal = texture(texture_atlas_normal, texCoords).rgb * 2.0 - 1.0;
    Normal = normalize(vs_in.TBN * Normal);
        
    vec3 F0 = F0calc(Mer.r, specular, Albedo.rgb);
    vec3 ViewDir = normalize(cam.position.xyz - vs_in.WorldPos);

    //Dir Shadow calculation
    float shadow = 1.0;

    vec3 viewSpacePos = (cam.view * vec4(vs_in.WorldPos, 1.0)).xyz;

    shadow = ShadowCalculation(viewSpacePos, vs_in.WorldPos.xyz, vec3(0.0), vec3(0.0));

     vec3 Lighting = vec3(0.0);
    //CALCULATE DIR LIGHT
    Lighting = shadow * calculate_light(scene.dirLightColor.rgb * scene.dirLightAmbientIntensity, normalize(scene.dirLightDirection.xyz), Normal, ViewDir, Albedo.rgb, 1.0, scene.dirLightSpecularIntensity, Mer.b, Mer.r, F0);

     //CALCULATE AMBIENT LIGHT
    vec3 Ambient = IBL_Calc(Mer.r, Mer.b, Normal.rgb, ViewDir.rgb, Albedo.rgb, F0);
