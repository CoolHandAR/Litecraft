...
void main(void)
{
	...
	if( ( ( LastMipSize.x & 1 ) != 0 && ( LastMipSize.y & 1 ) != 0) &&
        int(gl_FragCoord.x) == LastMipSize.x - 3 && int(gl_FragCoord.y) == LastMipSize.y - 3)
	{
		maxZ = max(maxZ, textureOffset(LastMip, TexCoord, ivec2(2, 2)).x);
	}
