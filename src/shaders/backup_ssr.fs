...
{   
  ...
  vec2 texSize  = vec2(1280 / 2, 720 / 2);
  vec2 texCoord = gl_FragCoord.xy / texSize;

  vec4 uv = vec4(0.0);

  vec4 positionFrom = calcViewPos(v_texCoords);
  vec4 mask         = texture(g_normal,     texCoord);
  ...
  vec3 normal           = texture(g_normal, texCoord).xyz * 2.0 -1.0;
  ...
  vec3 color = texture(g_color, uv.rg).rgb;
